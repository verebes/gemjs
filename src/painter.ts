...
export class Painter {
    ...
    static paint(scene: Scene, element: SVGElement): void {
        ...
        scene.shapes.forEach(
            shape => shape.getSides().forEach(
                side => polygons = polygons.concat(side)));

        let distances = polygons.map(Centroid.getCentroid).map(p => Vector.dot(p.toVector(), Vector.norm(camera.plane.normal)));
        ...
        idx.forEach((idx) => Painter.paintPolygon(scene, polygons[idx], element));
    }
